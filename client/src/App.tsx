...
function ConfirmDialog({ title, message, confirmLabel, onCancel, onConfirm }: { title: string; message: string; confirmLabel: string; onCancel: () => void; onConfirm: () => void }) {
  return <div className="modal-backdrop"><div className="confirm-dialog"><p className="eyebrow">Konfirmasi tindakan</p><h3>{title}</h3><p>{message}</p><div className="confirm-actions"><button className="outline-button" onClick={onCancel}>Batal</button><button className="danger-button" onClick={onConfirm}>{confirmLabel}</button></div></div></div>;
}
...
function AccountManagementPage() {
  ...
  const resetPassword = trpc.accounts.resetPassword.useMutation({ onSuccess: () => { setResetTarget(null); setResetPassword(""); refetch(); } });
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ username: "", name: "", password: "" });
  const [deactivateTarget, setDeactivateTarget] = useState<{ id: number; name: string } | null>(null);
  const [resetTarget, setResetTarget] = useState<{ id: number; name: string } | null>(null);
  const [resetPasswordValue, setResetPassword] = useState("");
  return <><PageIntro eyebrow="Kontrol akses" title={<>Kelola akun <em>petugas.</em></>} description="Buat akun lapangan, atur akses masuk, dan nonaktifkan akun yang tidak lagi bertugas." action={<button className="primary-button" onClick={() => setShowForm((value) => !value)}><UserCog size={16} /> {showForm ? "Tutup form" : "Buat akun petugas"}</button>} />
    ...
    <div className="panel table-panel account-table"><div className="table-heading"><div><p className="eyebrow">{accounts?.length ?? "—"} akun terdaftar</p><h3>Akun petugas lapangan</h3></div><span className="storage-note"><ShieldCheck size={13} /> Akses terkontrol</span></div><div className="data-table"><div className="table-row table-header"><span>Petugas</span><span>Username</span><span>Dibuat</span><span>Status</span><span>Aksi</span></div>{!accounts?.length ? <div className="empty-table">Belum ada akun petugas. Buat akun pertama dari tombol di atas.</div> : accounts.map((account) => <div className="table-row" key={account.id}><div className="person-cell"><div className="avatar avatar-lime">{(account.name ?? account.username ?? "P").split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</div><span><strong>{account.name ?? "Tanpa nama"}</strong><small>Petugas lapangan</small></span></div><span className="muted-cell">{account.username}</span><span className="muted-cell">{new Date(account.createdAt).toLocaleDateString("id-ID")}</span><span><StatusPill tone={account.isActive ? "success" : "process"}>{account.isActive ? "Aktif" : "Nonaktif"}</StatusPill></span><div className="account-actions"><button className="outline-button small" onClick={() => { setResetTarget({ id: account.id, name: account.name ?? account.username ?? "Petugas" }); setResetPassword(""); }}>Reset password</button><button className="outline-button small" disabled={setActive.isPending} onClick={() => account.isActive ? setDeactivateTarget({ id: account.id, name: account.name ?? account.username ?? "Petugas" }) : setActive.mutate({ id: account.id, isActive: true })}>{account.isActive ? "Nonaktifkan" : "Aktifkan"}</button></div></div>)}</div></div>
    {deactivateTarget && <ConfirmDialog title="Nonaktifkan akun petugas?" message={`Akun ${deactivateTarget.name} tidak dapat login dan tidak akan menerima akses tugas sampai diaktifkan kembali.`} confirmLabel="Nonaktifkan akun" onCancel={() => setDeactivateTarget(null)} onConfirm={() => { setActive.mutate({ id: deactivateTarget.id, isActive: false }); setDeactivateTarget(null); }} />}
    {resetTarget && <div className="modal-backdrop"><form className="confirm-dialog" onSubmit={(event) => { event.preventDefault(); resetPassword.mutate({ id: resetTarget.id, password: resetPasswordValue }); }}><p className="eyebrow">Reset kredensial</p><h3>Reset password {resetTarget.name}</h3><p>Password baru akan menggantikan password lama dan akun akan diaktifkan kembali.</p><label>Password baru<input autoFocus required minLength={12} type="password" value={resetPasswordValue} onChange={(event) => setResetPassword(event.target.value)} placeholder="Minimal 12 karakter" /></label>{resetPassword.error && <p className="form-error">{resetPassword.error.message}</p>}<div className="confirm-actions"><button type="button" className="outline-button" onClick={() => setResetTarget(null)}>Batal</button><button type="submit" className="primary-button" disabled={resetPassword.isPending}>{resetPassword.isPending ? "Menyimpan…" : "Simpan password"}</button></div></form></div>}</>;
}
...
function PetugasTaskPage({ compact = false }: { compact?: boolean }) {
  ...
  const filteredRows = rows.filter(({ report, location }) => {
    const status = report.status === "completed" ? "Selesai" : report.status === "review" ? "Review" : "Dalam proses";
    const haystack = `${location?.name ?? ""} ${location?.province ?? ""} ${report.officerName}`.toLowerCase();
    return (statusFilter === "Semua status" || status === statusFilter) && haystack.includes(query.toLowerCase().trim());
  });
