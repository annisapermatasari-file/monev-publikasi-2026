      ...
      <section className="auth-form-side">
        <div className="auth-form-wrap">
          ...
          <div className="auth-form-heading">
            ...
            <h2>Selamat datang<br /><span>kembali.</span></h2>
          ...
          <div className="demo-note"><Sparkles size={15} /><span><strong>Mode demo aktif.</strong> Gunakan kredensial yang sudah terisi untuk melihat dashboard.</span></div>
    ...
    <div className="app-layout">
      ...
      <aside className={`app-sidebar ${mobileOpen ? "is-open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          <div className="account-mini"><div className="avatar avatar-lime">SA</div><div><strong>Sari Anindita</strong><small>Super Admin</small></div><MoreHorizontal size={17} /></div>
          <button className="signout-button" onClick={signOut}><LogOut size={16} /> Keluar</button>
        </div>
      </aside>
      <div className="app-content"><header className="app-topbar"><div><p className="topbar-kicker">Monev Publikasi <span>/</span> 2026</p><p className="topbar-date">Senin, 21 September 2026</p></div><div className="topbar-actions"><button className="icon-button" aria-label="Cari"><Search size={18} /></button><button className="icon-button notification" aria-label="Notifikasi"><Bell size={18} /><i /></button><div className="topbar-profile"><div className="avatar avatar-lime">SA</div><div><strong>Sari Anindita</strong><small>Super Admin</small></div></div></div></header><main className="page-content">{children}</main></div>
      {showToast && <div className="toast"><Check size={16} /> Sesi demo ditutup. <Link href="/login">Masuk kembali</Link></div>}
    </div>
  );
}
...
function DashboardPage() {
  return <>
    ...
    <section className="stat-grid">
      <StatCard label="Total lokasi" value="64" note="Titik pemantauan aktif" tone="lime" icon={<MapPin size={17} />} trend="+8,4%" />
      <StatCard label="Monev selesai" value="42" note="Dari 64 penugasan" tone="blue" icon={<ClipboardCheck size={17} />} trend="+12,1%" />
      <StatCard label="Publikasi tayang" value="186" note="Konten terverifikasi" tone="pink" icon={<Images size={17} />} trend="+18,6%" />
      <StatCard label="Kelengkapan data" value="96%" note="Kualitas laporan" tone="amber" icon={<Activity size={17} />} trend="+4,2%" />
    </section>
    <section className="dashboard-grid">
      <div className="panel chart-panel"><PanelHeading eyebrow="Capaian monitoring" title="Aktivitas monev" action="7 hari terakhir" /><div className="chart-legend"><span><i className="legend-lime" /> Selesai</span><span><i className="legend-muted" /> Dalam proses</span></div><div className="bar-chart">{[54, 68, 46, 82, 66, 93, 76].map((height, index) => <div className="bar-column" key={index}><div className="bar-stack"><span className="bar-progress" style={{ height: `${height}%` }} /><span className="bar-rest" style={{ height: `${100 - height}%` }} /></div><small>{["15 Sep", "16 Sep", "17 Sep", "18 Sep", "19 Sep", "20 Sep", "21 Sep"][index]}</small></div>)}</div><div className="chart-foot"><span><strong>42</strong> laporan selesai</span><span className="positive"><ArrowUpRight size={14} /> 12,1% dari minggu lalu</span></div></div>
...
function MonevPage() {
  ...
  const rows = [
    { location: "Kabupaten Sleman", province: "DI Yogyakarta", officer: "Ari Rahman", date: "21 Sep 2026", status: "Selesai", tone: "success" },
    { location: "Kota Bandung", province: "Jawa Barat", officer: "Nisa Sari", date: "20 Sep 2026", status: "Review", tone: "review" },
    { location: "Kabupaten Gowa", province: "Sulawesi Selatan", officer: "Dimas Lestari", date: "20 Sep 2026", status: "Selesai", tone: "success" },
    { location: "Kabupaten Bantul", province: "DI Yogyakarta", officer: "Siti Aminah", date: "19 Sep 2026", status: "Dalam proses", tone: "process" },
  ];
  const filtered = filter === "Semua status" ? rows : rows.filter((row) => row.status === filter);
  return <><PageIntro eyebrow="Monev lapangan" title={<>Monitor dari <em>lapangan.</em></>} description="Kelola penugasan, pantau kelengkapan laporan, dan review hasil monev dalam satu ruang kerja." action={<button className="primary-button"><ClipboardList size={16} /> Mulai monev baru</button>} /><div className="toolbar"><div className="search-field"><Search size={16} /><input placeholder="Cari lokasi atau petugas..." /></div><div className="filter-group">{["Semua status", "Selesai", "Review", "Dalam proses"].map((item) => <button key={item} className={filter === item ? "selected" : ""} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="panel table-panel"><div className="table-heading"><div><p className="eyebrow">64 total penugasan</p><h3>Daftar monev</h3></div><button className="outline-button small">Export CSV <ArrowDownRight size={15} /></button></div><div className="data-table"><div className="table-row table-header"><span>Lokasi</span><span>Petugas</span><span>Tanggal</span><span>Status</span><span /></div>{filtered.map((row) => <div className="table-row" key={row.location}><div><strong>{row.location}</strong><small>{row.province}</small></div><div className="person-cell"><div className="avatar avatar-muted">{row.officer.split(" ").map((name) => name[0]).join("")}</div><span>{row.officer}</span></div><span className="muted-cell">{row.date}</span><span><StatusPill tone={row.tone}>{row.status}</StatusPill></span><button className="row-more" aria-label="Buka detail"><MoreHorizontal size={17} /></button></div>)}</div></div></>;
}

function DocumentationPage() {
  return <><PageIntro eyebrow="Dokumentasi publikasi" title={<>Bukti yang <em>berbicara.</em></>} description="Koleksi dokumentasi kegiatan dan publikasi yang telah diverifikasi oleh tim lapangan." action={<button className="primary-button"><Images size={16} /> Tambah dokumentasi</button>} /><div className="doc-metrics"><div><span className="metric-icon pink-bg"><Images size={17} /></span><div><strong>186</strong><small>Total dokumentasi</small></div></div><div><span className="metric-icon lime-bg"><Check size={17} /></span><div><strong>154</strong><small>Terverifikasi</small></div></div><div><span className="metric-icon blue-bg"><Radio size={17} /></span><div><strong>32</strong><small>Menunggu review</small></div></div></div><div className="panel gallery-panel"><div className="table-heading"><div><p className="eyebrow">Terbaru diunggah</p><h3>Galeri dokumentasi</h3></div><button className="outline-button small">Filter <ChevronRight size={15} /></button></div><div className="gallery-grid">{["Kelas tata boga PKK", "Pelatihan digital PKW", "Pameran karya warga", "Workshop wirausaha", "Kunjungan lapangan", "Forum kolaborasi"].map((title, index) => <div className={`gallery-card gallery-${index + 1}`} key={title}><div className="gallery-art"><span>{String(index + 1).padStart(2, "0")}</span><Images size={28} /></div><div className="gallery-copy"><strong>{title}</strong><small>{["Kab. Sleman", "Kota Bandung", "Kab. Gowa", "Kab. Bantul", "Kota Surakarta", "Kab. Bogor"][index]} <span>·</span> 2 hari lalu</small></div></div>)}</div></div></>;
}

function ReviewPage() {
  return <><PageIntro eyebrow="Kontrol kualitas" title={<>Review hasil <em>monev.</em></>} description="Pastikan setiap laporan lapangan telah lengkap, terverifikasi, dan siap menjadi insight." action={<button className="outline-button"><FileText size={16} /> Panduan review</button>} /><div className="review-banner"><div className="review-banner-icon"><ShieldCheck size={22} /></div><div><strong>12 laporan menunggu review</strong><p>Prioritaskan laporan yang masuk lebih dari 24 jam lalu.</p></div><button className="light-button">Mulai review <ArrowUpRight size={16} /></button></div><div className="review-grid"><div className="panel table-panel"><div className="table-heading"><div><p className="eyebrow">Antrian Anda</p><h3>Perlu ditinjau</h3></div><span className="count-badge">12</span></div><div className="review-list">{["Kota Bandung", "Kabupaten Bantul", "Kabupaten Bogor"].map((name, index) => <div className="review-row" key={name}><div className={`review-marker marker-${index}`} /><div><strong>{name}</strong><small>{["Nisa Sari", "Siti Aminah", "Rizki Fadillah"][index]} <span>·</span> {index + 1} jam lalu</small></div><StatusPill tone="review">Review</StatusPill><ChevronRight size={16} /></div>)}</div></div><div className="panel score-panel"><PanelHeading eyebrow="Kualitas data" title="Skor kelengkapan" action="Detail" /><div className="score-display"><span>96</span><small>/ 100</small></div><p>Rata-rata kelengkapan laporan nasional.</p><div className="score-bar"><span /></div><div className="score-foot"><span><i className="dot lime-dot" /> Sangat baik</span><span>+4,2% bulan ini</span></div></div></div></>;
}

function LocationsPage() {
  return <><PageIntro eyebrow="Titik pemantauan" title={<>Baca peta <em>kegiatan.</em></>} description="Atur lokasi monev, penanggung jawab, dan status pemantauan di setiap wilayah." action={<button className="primary-button"><MapPin size={16} /> Tambah lokasi</button>} /><div className="location-layout"><div className="location-map panel"><div className="map-grid" /><div className="map-rings" /><div className="map-label label-a"><i /> Jawa Barat <strong>18</strong></div><div className="map-label label-b"><i /> DI Yogyakarta <strong>12</strong></div><div className="map-label label-c"><i /> Sulawesi Selatan <strong>9</strong></div><div className="map-center"><Radio size={16} /><span>64 lokasi aktif</span></div></div><div className="panel location-list"><PanelHeading eyebrow="Lokasi aktif" title="Semua wilayah" action="Kelola" />{["Jawa Barat", "Jawa Tengah", "DI Yogyakarta", "Sulawesi Selatan", "Jawa Timur"].map((name, index) => <div className="location-row" key={name}><div className={`location-pin pin-${index}`}><MapPin size={14} /></div><div><strong>{name}</strong><small>{[18, 15, 12, 9, 7][index]} titik pemantauan</small></div><ChevronRight size={16} /></div>)}</div></div></>;
}
